...
mod tests {
    ...
    fn write_hook_plugin(
        ...
        let (extension, pre_script, post_script, failure_script) = if cfg!(windows) {
            (
                "cmd",
        ...
        fs::write(
            root.join("hooks").join(format!("failure.{extension}")),
            failure_script,
        )
