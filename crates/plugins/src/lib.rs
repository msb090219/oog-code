...
impl PluginManager {
    #[must_use]
    pub fn new(mut config: PluginManagerConfig) -> Self {
        for (plugin_id, enabled) in load_persisted_enabled_plugins(&config.config_home) {
            config.enabled_plugins.entry(plugin_id).or_insert(enabled);
        }
        Self { config }
    }
...
fn run_lifecycle_commands(
    ...
    for command in commands {
        let command_path = metadata.root.as_ref().map(|root| root.join(command));
        let mut process = if command_path.as_ref().is_some_and(|path| path.exists()) {
            let path = command_path.as_ref().expect("checked plugin command path");
...
fn load_persisted_enabled_plugins(config_home: &Path) -> BTreeMap<String, bool> {
    let path = config_home.join(SETTINGS_FILE_NAME);
...
mod tests {
  ...
  "commands": [
    ...
    fn write_lifecycle_plugin(root: &Path, name: &str, version: &str) -> PathBuf {
        ...
        write_file(
            root.join("lifecycle")
                .join(format!("init.{extension}"))
                .as_path(),
            init_script,
        );
        write_file(
            root.join("lifecycle")
                .join(format!("shutdown.{extension}"))
                .as_path(),
            shutdown_script,
        );
        let lifecycle_paths = format!(
            "\"Init\": [\"./lifecycle/init.{extension}\"],\n    \"Shutdown\": [\"./lifecycle/shutdown.{extension}\"]"
        );
        write_file(
            root.join(MANIFEST_RELATIVE_PATH).as_path(),
            format!(
    ...
    fn write_tool_plugin_with_name(root: &Path, name: &str, version: &str, tool_name: &str) {
        let (extension, script) = if cfg!(windows) {
            (
                "cmd",
        ...
        let script_path = root.join("tools").join(format!("echo-json.{extension}"));
        ...
        write_file(
            root.join(MANIFEST_RELATIVE_PATH).as_path(),
            format!(
                "{{\n  \"name\": \"{name}\",\n  \"version\": \"{version}\",\n  \"description\": \"tool plugin\",\n  \"tools\": [\n    {{\n      \"name\": \"{tool_name}\",\n      \"description\": \"Echo JSON input\",\n      \"inputSchema\": {{\"type\": \"object\", \"properties\": {{\"message\": {{\"type\": \"string\"}}}}, \"required\": [\"message\"], \"additionalProperties\": false}},\n      \"command\": \"./tools/echo-json.{extension}\",\n      \"requiredPermission\": \"workspace-write\"\n    }}\n  ]\n}}"
            )
            .as_str(),
        );
    }
