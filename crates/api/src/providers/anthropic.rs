...
const API_KEY_ENV: &str = "TROGLODYTIC_API_KEY";
const LEGACY_API_KEY_ENV: &str = "MINSROPIC_API_KEY";
const BASE_URL_ENV: &str = "TROGLODYTIC_BASE_URL";
const LEGACY_BASE_URL_ENV: &str = "MINSROPIC_BASE_URL";
...
impl AuthSource {
    pub fn from_env() -> Result<Self, ApiError> {
        let api_key = read_env_non_empty(API_KEY_ENV)?.or(read_env_non_empty(LEGACY_API_KEY_ENV)?);
        ...
        match (api_key, auth_token) {
            ...
            (None, None) => Err(ApiError::missing_credentials(
                "Troglodytic",
                &["ANTHROPIC_AUTH_TOKEN", API_KEY_ENV, LEGACY_API_KEY_ENV],
            )),
        }
    }
...
impl AuthSource {
    pub fn from_env_or_saved() -> Result<Self, ApiError> {
        // Check the new Oog Code API key first, then the legacy name.
        if let Some(api_key) =
            read_env_non_empty(API_KEY_ENV)?.or(read_env_non_empty(LEGACY_API_KEY_ENV)?)
        {
        ...
        // Fall back to ANTHROPIC_API_KEY for compatibility
...
pub fn has_auth_from_env_or_saved() -> Result<bool, ApiError> {
    Ok(read_env_non_empty(API_KEY_ENV)?
        .or(read_env_non_empty(LEGACY_API_KEY_ENV)?)
        .is_some()
    ...
    F: FnOnce() -> Result<Option<OAuthConfig>, ApiError>,
{
    ...
    let Some(token_set) = load_saved_oauth_token()? else {
        return Err(ApiError::missing_credentials(
            "Troglodytic",
            &["ANTHROPIC_AUTH_TOKEN", API_KEY_ENV, "ANTHROPIC_API_KEY"],
        ));
    };
...
pub fn read_base_url() -> String {
    std::env::var(BASE_URL_ENV)
        .or_else(|_| std::env::var(LEGACY_BASE_URL_ENV))
...
mod tests {
    ...
    fn read_api_key_requires_presence() {
        ...
        std::env::remove_var("MINSROPIC_API_KEY");
        std::env::set_var(
            "MINSEO_CONFIG_HOME",
            std::env::temp_dir().join("minseo-test-empty-auth"),
        );
    ...
    fn auth_source_from_env_combines_api_key_and_bearer_token() {
        let _guard = env_lock();
        std::env::set_var("ANTHROPIC_AUTH_TOKEN", "auth-token");
        std::env::set_var("MINSROPIC_API_KEY", "minseo-key");
        let auth = AuthSource::from_env().expect("env auth");
        assert_eq!(auth.api_key(), Some("minseo-key"));
    ...
    fn auth_source_from_saved_oauth_when_env_absent() {
        ...
        std::env::remove_var("MINSEO_CONFIG_HOME");
