...
fn execute_skill(input: SkillInput) -> Result<SkillOutput, String> {
    ...
    Ok(SkillOutput {
        skill: input.skill,
        path: skill_path.to_string_lossy().replace('\\', "/"),
        args: input.args,
        description,
        prompt,
    })
...
fn config_file_for_scope(scope: ConfigScope) -> Result<PathBuf, String> {
    ...
    Ok(match scope {
        ...
        ConfigScope::Settings => cwd.join(".minseo").join("settings.local.json"),
    })
}

fn config_home_dir() -> Result<PathBuf, String> {
    if let Ok(path) =
        std::env::var("OOG_CONFIG_HOME").or_else(|_| std::env::var("MINSEO_CONFIG_HOME"))
    {
        return Ok(PathBuf::from(path));
    }
    let home = std::env::var("HOME").map_err(|_| String::from("HOME is not set"))?;
    Ok(PathBuf::from(home).join(".minseo"))
}
...
mod tests {
    ...
    use super::{
        agent_permission_policy, allowed_tools_for_subagent, classify_lane_failure, command_exists,
  ...
  "nbformat_minor": 5
    ...
    fn config_reads_and_writes_supported_values() {
        ...
        std::fs::create_dir_all(home.join(".minseo")).expect("home dir");
        std::fs::create_dir_all(cwd.join(".minseo")).expect("cwd dir");
        std::fs::write(
            home.join(".minseo").join("settings.json"),
            r#"{"verbose":false}"#,
        )
    ...
    fn enter_and_exit_plan_mode_round_trip_existing_local_override() {
        ...
        std::fs::create_dir_all(cwd.join(".minseo")).expect("cwd dir");
        std::fs::write(
            cwd.join(".minseo").join("settings.local.json"),
        ...
        let local_settings =
            std::fs::read_to_string(cwd.join(".minseo").join("settings.local.json"))
...
"#,
    ...
    fn given_no_enforcer_when_bash_then_executes_normally() {
        ...
        let result = registry
            .execute(
                "bash",
                &json!({ "command": if cfg!(windows) { "echo|set /p=ok" } else { "printf 'ok'" } }),
            )
