...
mod tests {
    ...
    fn shell_snippet(script: &str) -> String {
        match script {
            "printf 'pre ok'" => cmd_write("pre ok", 0),
            "printf 'blocked by hook'; exit 2" => cmd_write("blocked by hook", 2),
            "printf 'warning hook'; exit 1" => cmd_write("warning hook", 1),
            "printf 'failure hook ran'" => cmd_write("failure hook ran", 0),
            "printf 'broken failure hook'; exit 1" => cmd_write("broken failure hook", 1),
            "printf 'later failure hook'" => cmd_write("later failure hook", 0),
            "printf 'first'" => cmd_write("first", 0),
            "printf 'second'" => cmd_write("second", 0),
            "printf 'broken'; exit 1" => cmd_write("broken", 1),
            "printf 'later'" => cmd_write("later", 0),
            "sleep 5" => "powershell -NoProfile -Command \"Start-Sleep -Seconds 5\"".to_string(),
            r#"printf '%s' '{"systemMessage":"updated","hookSpecificOutput":{"permissionDecision":"allow","permissionDecisionReason":"hook ok","updatedInput":{"command":"git status"}}}'"# => {
                cmd_write(
    ...
    #[cfg(windows)]
    fn cmd_write(message: &str, exit_code: i32) -> String {
        let escaped = message
            .replace('^', "^^")
            .replace('"', "^\"")
            .replace('&', "^&")
            ...
            .replace('<', "^<")
            .replace('>', "^>");
        format!("<nul set /p dummy={escaped} & exit /b {exit_code}")
    }
