...
pub enum OutputMode {
    /// Minimal output - hide headers/announcements, auto-collapse
    #[default]
    Minimal,
    /// Normal output - show headers, smart collapse
    Normal,
    /// Verbose output - show everything, no auto-collapse
    Verbose,
}
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, Default)]
pub enum UiMode {
    /// Traditional REPL (rustyline)
    Repl,
    /// Full TUI (ratatui) - DEFAULT
    #[default]
    Tui,
}
...
/// Collapse thresholds for different content types
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct CollapseThresholds {
    /// Lines before collapsing code blocks
    pub code_block: usize,
    /// Lines before collapsing tool output
    pub tool_output: usize,
    /// Lines before collapsing thinking/praying
    pub thinking: usize,
}
...
pub struct RuntimeOutputConfig {
    ...
    pub ui_mode: UiMode,
    /// Collapse thresholds for different content types
    pub collapse_thresholds: CollapseThresholds,
    /// Code block preview lines
    pub code_preview_lines: usize,
}

impl Default for RuntimeOutputConfig {
    fn default() -> Self {
        Self {
            mode: OutputMode::default(),
            ui_mode: UiMode::default(),
            collapse_thresholds: CollapseThresholds::default(),
...
impl ConfigLoader {
    ...
    pub fn discover(&self) -> Vec<ConfigEntry> {
        ...
        vec![
            ...
            ConfigEntry {
                ...
                path: self.cwd.join(".minseo.json"),
            },
            ConfigEntry {
                source: ConfigSource::Project,
                path: self.cwd.join(".minseo").join("settings.json"),
            },
            ConfigEntry {
                source: ConfigSource::Local,
                path: self.cwd.join(".minseo").join("settings.local.json"),
            },
            ConfigEntry {
                source: ConfigSource::Project,
    ...
    pub fn load(&self) -> Result<RuntimeConfig, ConfigError> {
        ...
        let feature_config = RuntimeFeatureConfig {
            ...
            output: parse_optional_output_config(&merged_value)?,
        };
...
impl RuntimeFeatureConfig {
    ...
    #[must_use]
    pub fn output(&self) -> &RuntimeOutputConfig {
        &self.output
    }
}
...
pub fn default_config_home() -> PathBuf {
    std::env::var_os("OOG_CONFIG_HOME")
        .or_else(|| std::env::var_os("MINSEO_CONFIG_HOME"))
        .map(PathBuf::from)
        .or_else(|| std::env::var_os("HOME").map(|home| PathBuf::from(home).join(".minseo")))
        .unwrap_or_else(|| PathBuf::from(".minseo"))
}
...
fn parse_optional_output_config(root: &JsonValue) -> Result<RuntimeOutputConfig, ConfigError> {
    let Some(output_value) = root.as_object().and_then(|object| object.get("output")) else {
        return Ok(RuntimeOutputConfig::default());
    };
    let object = expect_object(output_value, "merged settings.output")?;

    // Parse output mode
    let mode = if let Some(mode_value) = object.get("mode") {
        let mode_str = mode_value.as_str().ok_or_else(|| {
            ConfigError::Parse("merged settings.output.mode: expected string".to_string())
        })?;
        OutputMode::parse(mode_str)
            .ok_or_else(|| ConfigError::Parse(format!(
                "merged settings.output.mode: invalid output mode '{mode_str}' (expected: minimal, normal, verbose)"
            )))?
    } else {
        OutputMode::default()
    };
    ...
    // Parse UI mode
    let ui_mode = if let Some(ui_mode_value) = object.get("uiMode") {
        let ui_mode_str = ui_mode_value.as_str().ok_or_else(|| {
            ConfigError::Parse("merged settings.output.uiMode: expected string".to_string())
        })?;
        UiMode::parse(ui_mode_str)
            .ok_or_else(|| ConfigError::Parse(format!(
                "merged settings.output.uiMode: invalid UI mode '{ui_mode_str}' (expected: repl, tui)"
            )))?
    } else {
        UiMode::default()
    };

    // Parse collapse thresholds
    let collapse_thresholds = if let Some(thresholds_value) = object.get("collapseThresholds") {
        let thresholds = expect_object(
            thresholds_value,
            "merged settings.output.collapseThresholds",
        )?;
        CollapseThresholds {
            code_block: optional_u64(
                thresholds,
                "codeBlock",
                "merged settings.output.collapseThresholds",
            )?
            .map_or(20, |v| usize::try_from(v).unwrap_or(20)),
            tool_output: optional_u64(
                thresholds,
                "toolOutput",
                "merged settings.output.collapseThresholds",
            )?
            .map_or(60, |v| usize::try_from(v).unwrap_or(60)),
            thinking: optional_u64(
                thresholds,
                "thinking",
                "merged settings.output.collapseThresholds",
            )?
    ...
    // Parse code preview lines
    let code_preview_lines = optional_u64(object, "codePreviewLines", "merged settings.output")?
        .map_or(12, |v| usize::try_from(v).unwrap_or(12));
