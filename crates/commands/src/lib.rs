...
const SLASH_COMMAND_SPECS: &[SlashCommandSpec] = &[
    ...
    SlashCommandSpec {
        name: "project",
        aliases: &[],
        summary: "Show current project and workspace context",
        argument_hint: None,
        resume_supported: true,
    },
    SlashCommandSpec {
    ...
    SlashCommandSpec {
        name: "output",
        aliases: &["out"],
        summary: "Show or switch output mode (minimal|normal|verbose)",
    ...
    SlashCommandSpec {
        name: "session",
        aliases: &[],
        summary: "List, title, switch, fork, or delete managed local sessions",
        argument_hint: Some("[list|rename <title>|switch <session-id>|fork [branch-name]|delete <session-id> --confirm]"),
        resume_supported: false,
    },
    SlashCommandSpec {
        name: "plugin",
        aliases: &["plugins", "marketplace"],
        summary: "Manage Oog Code plugins",
        argument_hint: Some(
    ...
    SlashCommandSpec {
        name: "shortcuts",
        aliases: &["?"],
        summary: "Show keyboard shortcut reference",
        argument_hint: None,
        resume_supported: true,
    },
    SlashCommandSpec {
...
pub fn validate_slash_command_input(
    ...
    Ok(Some(match command {
        ...
        "output" | "out" => SlashCommand::Output {
            mode: optional_single_arg(command, &args, "[mode]")?,
        },
        "debug-tool-call" => {
            validate_no_args(command, &args)?;
        ...
        "verbosity" => SlashCommand::Verbosity { level: remainder },
...
fn parse_session_command(args: &[&str]) -> Result<SlashCommand, SlashCommandParseError> {
    match args {
        ...
        ["list", ..] => Err(usage_error("session", "[list|rename <title>|switch <session-id>|fork [branch-name]|delete <session-id> --confirm]")),
        ["rename"] => Err(usage_error("session rename", "<title>")),
        ["rename", title @ ..] => Ok(SlashCommand::Session {
            action: Some("rename".to_string()),
            target: Some(title.join(" ")),
        }),
        ...
        ["delete", target, "--confirm"] => Ok(SlashCommand::Session {
            action: Some("delete".to_string()),
            target: Some((*target).to_string()),
        }),
        ["delete", ..] => Err(usage_error("session delete", "<session-id> --confirm")),
        [action, ..] => Err(command_error(
            &format!(
                "Unknown /session action '{action}'. Use list, rename <title>, switch <session-id>, fork [branch-name], or delete <session-id> --confirm."
            ),
            "session",
            "/session [list|rename <title>|switch <session-id>|fork [branch-name]|delete <session-id> --confirm]",
        )),
    }
}
...
fn slash_command_category(name: &str) -> &'static str {
    match name {
        "help" | "status" | "project" | "sandbox" | "model" | "permissions" | "cost" | "resume"
...
mod tests {
    ...
    #[test]
    fn retired_renderer_command_is_unknown() {
        assert_eq!(
            SlashCommand::parse("/renderer on"),
            Ok(Some(SlashCommand::Unknown("renderer".to_string())))
        );
        assert!(!slash_command_specs()
            .iter()
            .any(|spec| spec.name == "renderer"));
    }
    ...
    fn parses_supported_slash_commands() {
        ...
        assert_eq!(
            SlashCommand::parse("/session rename Fix login flow"),
            Ok(Some(SlashCommand::Session {
                action: Some("rename".to_string()),
                target: Some("Fix login flow".to_string())
            }))
        );
        assert_eq!(
            SlashCommand::parse("/session delete abc123 --confirm"),
            Ok(Some(SlashCommand::Session {
                action: Some("delete".to_string()),
                target: Some("abc123".to_string())
            }))
    ...
    fn renders_per_command_help_detail() {
        ...
        assert!(help.contains("Summary          Manage Oog Code plugins"));
        assert!(help.contains("Aliases          /plugins, /marketplace"));
