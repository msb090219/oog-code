...
mod completion;
mod format; // Phase 0.1: Extracted formatting functions
mod init;
...
mod render;
mod session_manager; // Phase 0.1: Extracted session management
#[allow(
...
const DEFAULT_DATE: &str = "2026-04-05";
...
fn with_terminal_output<F>(write_output: F) -> io::Result<()>
where
    F: FnOnce(&mut dyn Write) -> io::Result<()>,
{
    let lock = TERMINAL_OUTPUT_LOCK.get_or_init(|| Mutex::new(()));
    let _guard = lock
        .lock()
...
fn main() {
    if let Err(error) = run() {
        ...
        if message.contains("`oog --help`") {
            eprintln!("error: {message}");
        } else {
            eprintln!(
                "error: {message}

Run `oog --help` for usage."
            );
        }
        std::process::exit(1);
    }
...
fn bare_slash_command_guidance(command_name: &str) -> Option<String> {
    ...
    let slash_command = slash_command_specs().iter().find(|spec| {
        is_live_slash_command(spec.name)
            && (spec.name == command_name || spec.aliases.contains(&command_name))
    })?;
    let guidance = if slash_command.resume_supported {
        format!(
            "`oog {command_name}` is a slash command. Use `oog --resume SESSION.jsonl /{command_name}` or start `oog` and run `/{command_name}`."
        )
    } else {
        format!(
            "`oog {command_name}` is a slash command. Start `oog` and run `/{command_name}` inside the REPL."
        )
    };
    Some(guidance)
}
...
fn parse_direct_slash_cli_action(rest: &[String]) -> Result<CliAction, String> {
    ...
    match SlashCommand::parse(&raw) {
        ...
        Ok(Some(command)) => Err({
            let _ = command;
            format!(
                "slash command {command_name} is interactive-only. Start `oog` and run it there, or use `oog --resume SESSION.jsonl {command_name}` / `oog --resume {latest} {command_name}` when the command is marked [resume] in /help.",
                command_name = rest[0],
...
/// Convert a crossterm Color to an ANSI escape sequence
fn color_to_ansi(color: crossterm::style::Color) -> String {
    match color {
        crossterm::style::Color::Rgb { r, g, b } => {
            format!("\x1b[38;2;{r};{g};{b}m")
        }
        crossterm::style::Color::AnsiValue(val) => {
            format!("\x1b[38;5;{val}m")
        }
        ...
        crossterm::style::Color::Cyan => "\x1b[36m".to_string(),
        crossterm::style::Color::White => "\x1b[37m".to_string(),
        crossterm::style::Color::DarkGrey => "\x1b[90m".to_string(),
        crossterm::style::Color::DarkRed => "\x1b[31m".to_string(),
        crossterm::style::Color::DarkGreen => "\x1b[32m".to_string(),
        crossterm::style::Color::DarkYellow => "\x1b[33m".to_string(),
        crossterm::style::Color::DarkBlue => "\x1b[34m".to_string(),
        crossterm::style::Color::DarkMagenta => "\x1b[35m".to_string(),
        crossterm::style::Color::DarkCyan => "\x1b[36m".to_string(),
        crossterm::style::Color::Grey => "\x1b[37m".to_string(),
        _ => "\x1b[39m".to_string(), // Default color
    }
}

fn theme_primary_ansi() -> String {
    color_to_ansi(
        theme::Theme::new(theme::ThemePreset::Default)
            .semantic
            .primary,
    )
}

fn theme_secondary_ansi() -> String {
    color_to_ansi(
        theme::Theme::new(theme::ThemePreset::Default)
            .semantic
            .secondary,
...
fn render_turn_body(content: &str) {
    let body_width = terminal_width().saturating_sub(2).max(20);
    for line in content.lines() {
        if line.is_empty() {
            println!();
            continue;
        }
        for wrapped in layout::wrap_text(line, body_width).lines() {
            println!("> {wrapped}");
        }
    }
    if content.is_empty() {
...
fn indent_output_chunk(chunk: &str, trailing_newlines: &mut usize) -> String {
    *trailing_newlines = chunk
        .chars()
        .rev()
...
fn render_section_divider(label: &str) -> String {
    ...
    format!("{}{}{}\x1b[0m", theme_muted_ansi(), label, rule)
}

fn render_section_start(label: &str, trailing_newlines: &mut usize) -> String {
    let spacing = "\n".repeat(2usize.saturating_sub(*trailing_newlines));
    *trailing_newlines = 1;
    format!("{spacing}{}\n", render_section_divider(label))
}

fn render_input_prompt() -> String {
    " oog ‣ ".to_string()
}

fn render_user_message(content: &str) {
    let width = terminal_width().max(24);
    let body_width = width.saturating_sub(8).max(10);
    ...
    for paragraph in content.lines() {
        if paragraph.is_empty() {
            ...
            lines.extend(
                layout::wrap_text(paragraph, body_width)
                    .lines()
                    .map(str::to_owned),
            );
        }
    ...
    let background = "\x1b[48;5;238m\x1b[97m";
    let empty_row = " ".repeat(width);
    println!();
    println!("{background}{empty_row}\x1b[0m");
    for (index, line) in lines.iter().enumerate() {
        let prefix = if index == 0 { " user ‣ " } else { "        " };
        let padding =
            " ".repeat(width.saturating_sub(prefix.chars().count() + line.chars().count()));
        println!("{background}{prefix}{line}{padding}\x1b[0m");
    }
    println!("{background}{empty_row}\x1b[0m");
    println!();
}
...
fn render_assistant_header() {
    print!("{}• \x1b[0m", theme_primary_ansi());
    let _ = io::stdout().flush();
}

fn render_turn_divider(elapsed: Duration) {
    let elapsed = format_elapsed(elapsed);
    let label = format!("─ carved in {elapsed} ");
    let rule = "─".repeat(
        terminal_width()
            .saturating_sub(label.chars().count())
            .max(1),
    );
    println!("{}{}{}\x1b[0m", theme_muted_ansi(), label, rule);
    println!();
}
...
fn format_turn_result(
    ...
    let tool_uses = summary
        ...
        .filter_map(|block| match block {
            ContentBlock::ToolUse { name, input, .. } => Some((name.as_str(), input.as_str())),
    ...
    let changed_files = tool_uses
        .iter()
        .filter(|(name, _)| matches!(*name, "write_file" | "Write" | "edit_file" | "Edit"))
        .map(|(_, input)| {
        ...
        .collect::<BTreeSet<_>>();
    let test_commands = tool_uses
        .iter()
        .filter(|(name, input)| {
            matches!(*name, "bash" | "Bash")
    ...
    let tests_failed = summary
        ...
        .any(|block| match block {
            ContentBlock::ToolResult {
                ...
                *is_error || bash_exit_code(output).is_some_and(|code| code != 0)
            }
            ContentBlock::Text { .. }
    ...
    let mut details = vec![format!("{} tools", tool_uses.len())];
    ...
    if test_commands > 0 {
        details.push(format!(
            "tests {}",
            if tests_failed { "failed" } else { "passed" }
        ));
    }
    details.push(format_elapsed(elapsed));
    details.push(format!(
        "{} in / {} out",
        summary.usage.input_tokens, summary.usage.output_tokens
    ));
    details.push(format_usd(
        summary
            .usage
            .estimate_cost_usd_with_pricing(pricing)
            .total_cost_usd(),
    ));
    Some(format!("Oog done · {}", details.join(" · ")))
}

fn bash_exit_code(output: &str) -> Option<u64> {
...
fn visible_message_text(message: &ConversationMessage) -> String {
    message
        .blocks
        .iter()
        .filter_map(|block| match block {
...
fn render_saved_session_transcript(session: &Session) -> io::Result<()> {
    let mut stdout = io::stdout();
    execute!(stdout, Clear(ClearType::All), MoveTo(0, 0))?;

    for message in &session.messages {
        let content = visible_message_text(message);
        if content.trim().is_empty() {
            continue;
        }

        match message.role {
            MessageRole::User => render_user_message(&content),
            MessageRole::Assistant => {
                render_assistant_header();
                print!("{}", TerminalRenderer::new().markdown_to_ansi(&content));
...
/// Get the terminal width, with a fallback value
...
fn format_unknown_direct_slash_command(name: &str) -> String {
    ...
    message.push_str("\nRun `oog --help` for CLI usage, or start `oog` and use /help.");
    message
}
...
fn suggest_slash_commands(input: &str) -> Vec<String> {
    let mut candidates = slash_command_specs()
        .iter()
        .filter(|spec| is_live_slash_command(spec.name))
        .flat_map(|spec| {
...
fn filter_tool_specs(
    ...
    tool_registry
        ...
        .into_iter()
        .filter(|tool| exposed_tools.contains(&tool.name))
        .collect()
}

fn default_tool_profile(allowed_tools: Option<&AllowedToolSet>) -> BTreeSet<String> {
    CORE_TOOL_NAMES
        .iter()
        .filter(|name| allowed_tools.is_none_or(|allowed| allowed.contains(**name)))
...
fn format_small_report(
    ...
    let active_tools = filter_tool_specs(tool_registry, allowed_tools, exposed_tools);
    let schema_tokens = active_tools
        .iter()
        .filter_map(|tool| serde_json::to_string(tool).ok())
        .map(|schema| estimate_text_tokens(&schema))
        .sum::<usize>();
    let (history_tokens, tool_result_tokens) =
        session
            .messages
            .iter()
            .fold((0_usize, 0_usize), |(history, tool_results), message| {
                let tokens = estimate_message_tokens(message);
                if message.role == MessageRole::Tool {
                    (history, tool_results + tokens)
                } else {
                    (history + tokens, tool_results)
                }
            });
    ...
    format!(
        "Small\n  Active tools     {}\n  Tool names       {active_names}\n\nProvider tokens (reported)\n  Latest input     {}\n  Latest output    {}\n  Latest cache in  {}\n  Latest cache out {}\n  Session input    {}\n  Session output   {}\n\nNext request (local estimate)\n  System prompt    ~{system_tokens}\n  Tool schemas     ~{schema_tokens}\n  History          ~{history_tokens}\n  Tool results     ~{tool_result_tokens}\n  Total            ~{}\n\nEstimates use text length; provider token counts above are authoritative.",
        active_tools.len(),
        latest.input_tokens,
        ...
        system_tokens + schema_tokens + history_tokens + tool_result_tokens,
    )
}
...
fn render_resume_usage() -> String {
  ...
  Auto-save        .minseo/sessions/<session-id>.{PRIMARY_SESSION_EXTENSION}
...
fn run_resume_command(
    ...
    match command {
        ...
        SlashCommand::Project => Ok(ResumeCommandOutcome {
            session: session.clone(),
            message: Some(format_project_report(
                "restored-session",
                default_permission_mode().as_str(),
                &status_context(Some(session_path))?,
            )),
        ...
        SlashCommand::Shortcuts => {
            // Handle shortcuts command
            Ok(ResumeCommandOutcome {
                session: session.clone(),
                message: Some(format_shortcuts_help()),
            })
        }
        SlashCommand::Output { .. } => {
            // Not yet implemented
            Err("command not yet implemented".into())
        }
    }
}
...
fn wait_for_repl_turn(events: &Receiver<ReplEvent>) -> bool {
    loop {
        match events.recv() {
            Ok(ReplEvent::Done) => return false,
            Ok(ReplEvent::Error(error)) => eprintln!("error: {error}"),
            Ok(ReplEvent::Exit) | Err(_) => return true,
        }
    }
}
...
fn run_repl(
    ...
    // Create LiveCli instance with full AI integration
    let cli = LiveCli::new(model.clone(), true, allowed_tools, permission_mode)?;

    // Show beautiful startup banner with purple theme and bearded emoji
    println!("{}", cli.startup_banner());

    // Create completion candidates for slash commands
    let completion_candidates =
        slash_command_completion_candidates_with_sessions(&model, Some(&cli.session.id), vec![]);

    let mut editor = LineEditor::new(render_input_prompt(), completion_candidates);
    ...
    let worker = thread::spawn(move || {
        ...
        while let Ok(request) = request_rx.recv() {
            match request {
                ReplRequest::Turn(input) => {
                    if let Err(error) = cli.run_turn(&input) {
                        let _ = event_tx.send(ReplEvent::Error(error.to_string()));
                    }
                    let _ = event_tx.send(ReplEvent::Done);
                }
                ReplRequest::Command(command) => match cli.handle_repl_command(command) {
                    Ok(true) => {
                        let _ = event_tx.send(ReplEvent::Exit);
                        break;
                    }
                    Ok(false) => {
                        let _ = event_tx.send(ReplEvent::Done);
                ...
                ReplRequest::Shutdown => break,
            }
        }
    });

    println!(
        "{}Type /help for commands. Ctrl+C or /quit to quit.\x1b[0m",
        theme_primary_ansi()
    );

    loop {
        let mut should_exit = false;
        let outcome = editor.read_line_with_live_dropdown_and(|| {
            while let Ok(event) = event_rx.try_recv() {
                match event {
                    ReplEvent::Done => {}
                    ReplEvent::Error(error) => eprintln!("error: {error}"),
                    ReplEvent::Exit => should_exit = true,
                }
            }
            Ok(())
        })?;
        ...
        match outcome {
            input::ReadOutcome::Submit(input) => {
                ...
                if let Ok(Some(command)) = commands::SlashCommand::parse(&input) {
                    request_tx
                        .send(ReplRequest::Command(command))
                        .map_err(|error| io::Error::new(io::ErrorKind::BrokenPipe, error))?;
                    if wait_for_repl_turn(&event_rx) {
                        break;
                    }
                    continue;
                }
            ...
            input::ReadOutcome::Exit => {
                let _ = request_tx.send(ReplRequest::Shutdown);
                break;
            }
        }
    }
    ...
    let _ = request_tx.send(ReplRequest::Shutdown);
    let _ = worker.join();
    println!("🪨 Goodbye! Thanks for using Oog Code!");
...
struct BuiltRuntime {
    ...
    mcp_active: bool,
    tool_registry: GlobalToolRegistry,
    exposed_tools: Arc<Mutex<BTreeSet<String>>>,
}
...
impl BuiltRuntime {
    ...
    fn active_tools(&self) -> BTreeSet<String> {
        self.exposed_tools
            .lock()
            .unwrap_or_else(std::sync::PoisonError::into_inner)
            .clone()
...
impl LiveCli {
    ...
    fn startup_banner(&self) -> String {
        ...
        use layout::{calculate_content_width, center_text};
        ...
        // Get theme colors with proper capability detection
        let theme = Theme::new(ThemePreset::Default);
        let primary = theme.semantic.primary;
        let secondary = theme.semantic.secondary;
        let muted = theme.semantic.muted;

        // Convert colors to ANSI
        let primary_ansi = color_to_ansi(primary);
        let secondary_ansi = color_to_ansi(secondary);
        let muted_ansi = color_to_ansi(muted);
        let reset = "\x1b[0m";
        ...
        // Legacy art buffer is replaced below by the Oog Code banner.
        banner.push_str(&primary_ansi);
        banner.push_str(
            "\
  ███╗   ███╗██╗███╗   ██╗███████╗███████╗ ██████╗\n\
  ████╗ ████║██║████╗  ██║██╔════╝██╔════╝██╔═══██╗\n\
...
",
        ...
        banner.push_str(reset);
        banner.clear();
        banner.push_str(&primary_ansi);
        banner.push_str("  ___    ___    ____    ");
        banner.push_str(&secondary_ansi);
        banner.push_str("  ____   ___   ____   _____\n");
        banner.push_str(&primary_ansi);
        banner.push_str(" / _ \\  / _ \\  / ___|   ");
        banner.push_str(&secondary_ansi);
        banner.push_str(" / ___| / _ \\ |  _ \\ | ____|\n");
        banner.push_str(&primary_ansi);
        banner.push_str("| | | || | | || |  _    ");
        banner.push_str(&secondary_ansi);
        banner.push_str("| |    | | | || | | ||  _|\n");
        banner.push_str(&primary_ansi);
        banner.push_str("| |_| || |_| || |_| |   ");
        banner.push_str(&secondary_ansi);
        banner.push_str("| |___ | |_| || |_| || |___\n");
        banner.push_str(&primary_ansi);
        banner.push_str(" \\___/  \\___/  \\____|   ");
        banner.push_str(&secondary_ansi);
        banner.push_str(" \\____| \\___/ |____/ |_____|\n");
        banner.push_str(&muted_ansi);
        banner.push_str("                 carve less. ship more.\n");
        banner.push_str(reset);

        // Empty line for spacing
        banner.push('\n');

        // Metadata rows with colored bullets (no box)
        let cwd_string = cwd;
        let model_string = self.model.clone();
        let permissions_string = self.permission_mode.as_str().to_string();

        let metadata: [(&str, &String); 3] = [
            ("Model", &model_string),
        ...
        for (label, value) in metadata {
            // Use Cerulean Blue bullet for user data
            let bullet = "●"; // Unicode bullet point
            let available_width = width.saturating_sub(4);

            // Truncate value if too long
            let display_value = if value.chars().count() > available_width {
            ...
            banner.push_str("  ");
            banner.push_str(&primary_ansi); // Cerulean Blue bullet
            banner.push_str(bullet);
            banner.push_str(reset);
            banner.push(' ');
            banner.push_str(&muted_ansi);
            banner.push_str(&format!("{label}:"));
            banner.push_str(reset);

            // Add spacing and value
            let label_width = label.chars().count() + 2; // +2 for ": "
            let spaces = " ".repeat(14usize.saturating_sub(label_width));
            banner.push_str(&spaces);
            banner.push_str(&display_value);
            banner.push('\n');
        }
        ...
        // Tip section (no box, just the tip text)
        let tip = "Ask Oog to build, fix, or explain something.";
        let tip_lines: Vec<&str> = tip.lines().collect();
        if !tip_lines.is_empty() {
            // Replace 💡 with 🙏 in tip
            let tip_text = tip_lines[0].replace("💡", "🙏");

            banner.push_str("  ");
            banner.push_str(&secondary_ansi);
            banner.push_str(&tip_text);
            banner.push_str(reset);
            banner.push('\n');
        }
        ...
        // Recent activity section (no box)
        let activity_text = "Saved chats: /session";
        banner.push_str("  ");
        banner.push_str(&muted_ansi);
        banner.push_str(activity_text);
        banner.push_str(reset);
        banner.push('\n');
    ...
    fn run_turn(&mut self, input: &str) -> Result<(), Box<dyn std::error::Error>> {
        ...
        match result {
            Ok(summary) => {
                ...
                if let Some(result) =
                    format_turn_result(&summary, turn_started.elapsed(), &self.model)
                {
                    println!("{result}");
                    println!();
                }
                ...
                render_turn_divider(turn_started.elapsed());
                self.persist_session()?;
                ensure_session_title(&self.session.path, self.runtime.session())?;
                Ok(())
            }
    ...
    fn handle_repl_command(
        ...
        Ok(match command {
            ...
            SlashCommand::Verbosity { level } => {
                self.set_verbosity(level.as_deref())?;
                false
            }
            SlashCommand::Agents { args } => {
            ...
            | SlashCommand::AddDir { .. } => {
                eprintln!(
                    "This command is not available in Oog Code. Run /help for available commands."
                );
                false
            }
    ...
    fn print_project(&self) {
        println!(
            "{}",
            format_project_report(
                &self.model,
                self.permission_mode.as_str(),
                &status_context(Some(&self.session.path)).expect("project context should load"),
            )
        );
    }
    ...
    fn set_verbosity(&mut self, level: Option<&str>) -> Result<bool, Box<dyn std::error::Error>> {
        ...
        let new_verbosity = Verbosity::parse(level).map_err(|e| e.clone())?;

        if new_verbosity == self.verbosity {
            println!(
                "Verbosity: {}\n\nLevels: terse, normal, verbose",
                new_verbosity.name()
            );
        ...
        let previous = self.verbosity;
        self.verbosity = new_verbosity;
        println!(
            "Verbosity: {} → {}\n\nChanges apply to the next tool execution.",
            previous.name(),
            self.verbosity.name()
        );
        Ok(false)
    ...
    fn print_small(&self) {
        let active_tools = self.runtime.active_tools();
        println!(
            "{}",
            format_small_report(
                &self.system_prompt,
    ...
    fn handle_shortcuts_command() -> Result<(), Box<dyn std::error::Error>> {
        println!("{}", format_shortcuts_help());
    ...
    fn handle_session_command(
        ...
        match action {
            None => match select_managed_session(&self.session.id)? {
                Some(session_id) => self.handle_session_command(Some("switch"), Some(&session_id)),
                None => Ok(false),
            },
            Some("list") => {
            ...
            Some("switch") => {
                ...
                render_saved_session_transcript(self.runtime.session())?;
                Ok(false)
            }
            Some("fork") => {
            ...
            Some("rename") => {
                let Some(title) = target else {
                    println!("Usage: /session rename <title>");
                    return Ok(false);
                };
                let title = normalize_session_title(title)?;
                write_session_title(&self.session.path, &title)?;
                println!(
                    "Session renamed\n  Title            {title}\n  Active session   {}",
                    self.session.id
                );
            ...
            Some("delete") => {
                let Some(target) = target else {
                    println!("Usage: /session delete <session-id> --confirm");
                    return Ok(false);
                };
                let handle = resolve_managed_session_handle(target)?;
                if handle.id == self.session.id {
                    println!("Cannot delete the active session. Switch to another session first.");
                    return Ok(false);
                }
                delete_managed_session(&handle)?;
                println!("Session deleted\n  Session          {}", handle.id);
                Ok(false)
            }
            Some(other) => {
                println!(
                    "Unknown /session action '{other}'. Use /session list, /session rename <title>, /session switch <session-id>, /session fork [branch-name], or /session delete <session-id> --confirm."
                );
                Ok(false)
            }
        }
...
fn sessions_dir() -> Result<PathBuf, Box<dyn std::error::Error>> {
    let cwd = env::current_dir()?;
    let path = cwd.join(".minseo").join("sessions");
...
fn resolve_managed_session_handle(
    session_id: &str,
) -> Result<SessionHandle, Box<dyn std::error::Error>> {
    Ok(SessionHandle {
        id: session_id.to_string(),
        path: resolve_managed_session_path(session_id)?,
    })
}

fn delete_managed_session(handle: &SessionHandle) -> Result<(), Box<dyn std::error::Error>> {
    fs::remove_file(&handle.path)?;
    let tool_output_path = handle.path.with_extension("tool-output");
    if tool_output_path.exists() {
        fs::remove_dir_all(tool_output_path)?;
    }
    let title_path = session_title_path(&handle.path);
...
fn session_title_path(session_path: &Path) -> PathBuf {
    session_path.with_extension("title")
}

fn normalize_session_title(title: &str) -> Result<String, Box<dyn std::error::Error>> {
    let title = title.trim();
    if title.is_empty() || title.contains(['\r', '\n']) {
        return Err("session title must be a single non-empty line".into());
    }
    Ok(title.chars().take(80).collect())
}

fn write_session_title(session_path: &Path, title: &str) -> Result<(), Box<dyn std::error::Error>> {
    fs::write(session_title_path(session_path), title)?;
    Ok(())
}

fn read_session_title(session_path: &Path) -> Option<String> {
    fs::read_to_string(session_title_path(session_path))
        .ok()
        .and_then(|title| normalize_session_title(&title).ok())
}

fn auto_session_title(session: &Session) -> Option<String> {
    session_preview(session).and_then(|preview| {
        preview
            .lines()
            .map(str::trim)
            .find(|line| !line.is_empty())
            .and_then(|line| normalize_session_title(line).ok())
    })
}

fn ensure_session_title(
    session_path: &Path,
    session: &Session,
) -> Result<(), Box<dyn std::error::Error>> {
    if read_session_title(session_path).is_none() {
        if let Some(title) = auto_session_title(session) {
            write_session_title(session_path, &title)?;
        }
    }
    Ok(())
}

fn session_preview(session: &Session) -> Option<String> {
    session.messages.iter().find_map(|message| {
        (message.role == MessageRole::User).then(|| {
            message.blocks.iter().find_map(|block| match block {
                ContentBlock::Text { text } if !text.trim().is_empty() => {
                    Some(truncate_for_summary(text.trim(), 96))
                }
                _ => None,
            })
        })?
...
fn list_managed_sessions() -> Result<Vec<ManagedSessionSummary>, Box<dyn std::error::Error>> {
    ...
    for entry in fs::read_dir(sessions_dir()?)? {
        ...
        let (id, message_count, preview, parent_session_id, branch_name) =
...
fn format_missing_session_reference(reference: &str) -> String {
    format!(
        "session not found: {reference}\nHint: managed sessions live in .minseo/sessions/. Try `{LATEST_SESSION_REFERENCE}` for the most recent session or `/session list` in the REPL."
    )
}

fn format_no_managed_sessions() -> String {
    format!(
        "no managed sessions found in .minseo/sessions/\nStart `oog` to create a session, then rerun with `--resume {LATEST_SESSION_REFERENCE}`."
    )
}

fn select_managed_session(
    ...
    let selection = (|| -> Result<Option<String>, Box<dyn std::error::Error>> {
        loop {
            let (_, terminal_rows) = terminal::size()?;
            let visible_sessions = session_picker_visible_count(terminal_rows);
            execute!(
                stdout,
                cursor::MoveTo(0, 0),
            ...
            write!(
                stdout,
                "{}",
                render_session_picker(&sessions, selected, confirming_delete, visible_sessions)
            )?;
            stdout.flush()?;
            ...
            if let event::Event::Key(key) = event::read()? {
                ...
                match key.code {
                    event::KeyCode::Char('y' | 'Y') if confirming_delete => {
                        let session = sessions.remove(selected);
                        delete_managed_session(&SessionHandle {
                            id: session.id,
                            path: session.path,
                        })?;
                        if sessions.is_empty() {
                    ...
                    event::KeyCode::Char('d' | 'D')
                        if sessions[selected].id != active_session_id =>
                    {
                        confirming_delete = true;
                    }
                    event::KeyCode::Up | event::KeyCode::Char('k') if !confirming_delete => {
                        selected = selected.saturating_sub(1);
                    }
                    event::KeyCode::Down | event::KeyCode::Char('j') if !confirming_delete => {
                        selected = (selected + 1).min(sessions.len() - 1);
                    }
                    event::KeyCode::PageUp if !confirming_delete => {
                        selected = selected.saturating_sub(visible_sessions);
                    }
                    event::KeyCode::PageDown if !confirming_delete => {
                        selected = (selected + visible_sessions).min(sessions.len() - 1);
                    }
                    event::KeyCode::Home if !confirming_delete => selected = 0,
                    event::KeyCode::End if !confirming_delete => selected = sessions.len() - 1,
                    event::KeyCode::Enter if !confirming_delete => {
                        return Ok(Some(sessions[selected].id.clone()));
                    }
                    event::KeyCode::Esc if confirming_delete => confirming_delete = false,
...
fn render_session_picker(
    ...
    let mut lines = vec![
        "Saved chats".to_string(),
        "Use ↑/↓ to select · Enter to open · Esc to cancel".to_string(),
        String::new(),
    ];
    lines[1] = if confirming_delete {
        "Delete selected chat? Y to confirm · Esc to cancel".to_string()
    } else {
        "Use ↑/↓ to select · Enter to open · D to delete · Esc to cancel".to_string()
    };
    let (start, end) = session_picker_window(selected, sessions.len(), visible_sessions);
    let current = if sessions.is_empty() {
        0
    } else {
        selected.saturating_add(1)
    };
    lines[0] = format!("Saved chats ({current}/{})", sessions.len());
    for (index, session) in sessions[start..end].iter().enumerate() {
        ...
        lines.push(format!(
            ...
            session.preview.as_deref().unwrap_or("No user prompt yet"),
        ));
        lines.push(format!("    {}", session.id));
        lines.push(String::new());
    }
    if sessions.len() > visible_sessions {
        lines.push(format!(
            "Showing {}–{} of {} chats",
            start + 1,
            end,
...
fn session_picker_visible_count(terminal_rows: u16) -> usize {
    usize::from(terminal_rows.saturating_sub(5) / 4).clamp(1, 7)
}
...
fn session_picker_window(
    ...
    let visible = visible_sessions.clamp(1, session_count);
    let start = selected
...
fn render_session_list(active_session_id: &str) -> Result<String, Box<dyn std::error::Error>> {
    ...
    for session in sessions {
        ...
        let title = session.title.as_deref().unwrap_or("Untitled chat");
        lines.push(format!(
            "  {marker} {id}  {title}\n    {msgs} messages · {modified}{lineage}\n    {preview}",
            id = session.id,
            msgs = session.message_count,
            modified = format_session_modified_age(session.modified_epoch_millis),
            lineage = lineage,
            preview = session.preview.as_deref().unwrap_or("No user prompt yet"),
        ));
    }
    Ok(lines.join("\n"))
}
...
fn render_repl_help() -> String {
    [
        "Oog controls".to_string(),
        "  /see                 Session, workspace, and active context".to_string(),
        "  /smash               Compact chat history".to_string(),
        "  /small               Token and context ledger".to_string(),
        "  /session             Open saved chats".to_string(),
        "  /clear --confirm     Start a fresh chat".to_string(),
        "  /cost                Provider token usage and cost".to_string(),
        "  /help                Show this guide".to_string(),
        "  /quit                Exit Oog".to_string(),
        String::new(),
        "Advanced controls".to_string(),
        "  /model [model]       Inspect or switch model".to_string(),
        "  /permissions [mode]  Inspect or switch permission mode".to_string(),
        String::new(),
        "Keys".to_string(),
        "  Up/Down              Prompt history".to_string(),
        "  Tab                  Complete commands and saved chats".to_string(),
        "  Ctrl-C               Clear input (or exit when empty)".to_string(),
        "  Shift+Enter/Ctrl+J   Insert a newline".to_string(),
    ]
    .join("\n")
}

fn is_live_slash_command(name: &str) -> bool {
    matches!(
        name,
        "help"
            | "see"
...
fn live_slash_command_category(name: &str) -> &'static str {
    match name {
        ...
        "agents" | "skills" | "teleport" | "debug-tool-call" | "mcp" | "shortcuts" => {
            "Discovery & debugging"
        }
        "bughunter" | "ultraplan" => "Analysis & automation",
        "exit" => "Communication & control",
        _ => "Other",
    }
}

fn format_live_slash_command_help_line(spec: &commands::SlashCommandSpec) -> String {
    ...
    format!("  {usage:<66} {}{alias_suffix}{resume}", spec.summary)
}

fn render_live_slash_command_help() -> String {
    let mut lines = vec![
    ...
    for category in [
        ...
        let matching = slash_command_specs()
            .iter()
            .filter(|spec| {
                is_live_slash_command(spec.name)
                    && live_slash_command_category(spec.name) == category
            })
            .collect::<Vec<_>>();
        ...
        for spec in matching {
            lines.push(format_live_slash_command_help_line(spec));
        }
        lines.push(String::new());
    }

    lines.push(
        "More commands are registered for future work, but hidden here until they are wired up."
            .to_string(),
    );
    lines.join("\n")
}

fn format_shortcuts_help() -> String {
...
fn format_project_report(model: &str, permission_mode: &str, context: &StatusContext) -> String {
    let project_root = context.project_root.as_deref().unwrap_or(&context.cwd);
    format!(
        "Project\n  Working directory {}\n  Project root      {}\n  Stack             {}\n  Git branch        {}\n  Git state         {}\n  Changed files     {} ({} staged, {} unstaged, {} untracked)\n  Instructions      {} loaded\n\nSession\n  Model             {model}\n  Permission mode   {permission_mode}\n  Saved chat        {}",
        context.cwd.display(),
        project_root.display(),
        detect_project_stack(project_root),
        context.git_branch.as_deref().unwrap_or("not a git repository"),
        context.git_summary.headline(),
        context.git_summary.changed_files,
        context.git_summary.staged_files,
        context.git_summary.unstaged_files,
...
fn detect_project_stack(root: &Path) -> String {
    let stacks = [
        ("Cargo.toml", "Rust"),
...
fn build_runtime_with_plugin_state(
    ...
    let exposed_tools = Arc::new(Mutex::new(default_tool_profile(allowed_tools.as_ref())));
...
impl ApiClient for AnthropicRuntimeClient {
    ...
    fn stream(&mut self, request: ApiRequest) -> Result<Vec<AssistantEvent>, RuntimeError> {
        ...
        self.runtime.block_on(async {
            ...
            let mut assistant_trailing_newlines = 0;
                ...
                match event {
                        ...
                        ContentBlockDelta::TextDelta { text } => {
                            if !text.is_empty() {
                                ...
                                if let Some(rendered) = markdown_stream.push(&renderer, &text) {
                                    let rendered = indent_output_chunk(
                                        &rendered,
                                        &mut assistant_trailing_newlines,
                                    );
                                    if self.emit_output {
                                        with_terminal_output(|out| {
                                            write!(out, "{rendered}")?;
                                            out.flush()
                                        })
                                        .map_err(|error| RuntimeError::new(error.to_string()))?;
                    ...
                    ApiStreamEvent::ContentBlockStop(_) => {
                        if let Some(rendered) = markdown_stream.flush(&renderer) {
                            let rendered =
                                indent_output_chunk(&rendered, &mut assistant_trailing_newlines);
                            if self.emit_output {
                                with_terminal_output(|out| {
                                    write!(out, "{rendered}")?;
                                    out.flush()
                        ...
                        if let Some((id, name, input)) = pending_tool.take() {
                            ...
                            let rendered = render_section_start(
...
fn format_tool_call_start(name: &str, input: &str) -> String {
    ...
    let detail = match name {
        "bash" | "Bash" => {
            ...
            format!("bonk: {}", truncate_for_summary(command, 100))
        }
        "read_file" | "Read" => {
        ...
        "glob_search" | "Glob" | "grep_search" | "Grep" => {
            ...
            let path = parsed
                ...
                .and_then(|value| value.as_str())
                .unwrap_or(".");
            format!("sniff: {pattern} @ {path}")
        }
        "web_search" | "WebSearch" => format!(
            "sniff web: {}",
            parsed
                .get("query")
                .and_then(|value| value.as_str())
        ...
        _ if parsed.get("command").is_some() => {
            let command = parsed
                .get("command")
                .and_then(|value| value.as_str())
                .unwrap_or("shell command");
...
fn format_tool_result(name: &str, output: &str, is_error: bool) -> String {
    let icon = if is_error {
        format!("{}x{}", theme_error_ansi(), "\x1b[0m")
    } else {
        format!("{}ok{}", theme_success_ansi(), "\x1b[0m")
    };
    if is_error {
        ...
        return if summary.is_empty() {
            ...
            format!("{icon} {name}: {}{summary}\x1b[0m", theme_error_ansi())
        };
    }
    ...
    match name {
        "bash" | "Bash" | "PowerShell" => format_shell_result(&icon, name, &parsed),
...
fn format_shell_result(icon: &str, name: &str, parsed: &serde_json::Value) -> String {
    let exit_code = parsed
        .get("exit_code")
        .or_else(|| parsed.get("exitCode"))
        .and_then(serde_json::Value::as_u64)
        .unwrap_or(0);
    format!("{icon} {name} finished (exit code {exit_code})")
}
...
fn format_edit_result(icon: &str, parsed: &serde_json::Value) -> String {
    ...
    // Try to get line changes from structured patch
    let (additions, deletions) =
        if let Some(hunks) = parsed.get("structuredPatch").and_then(|v| v.as_array()) {
            ...
            // Fallback: estimate from old/new string sizes
            let old_len = parsed
                .get("oldString")
            ...
            let new_len = parsed
                ...
                .and_then(|v| v.as_str())
                .map_or(0, |s| s.lines().count());
            (new_len.max(old_len) - old_len, 0)
        };

    format!("{icon} edit_file ({filename}: +{additions} -{deletions} lines)")
}
...
fn format_glob_result(icon: &str, parsed: &serde_json::Value) -> String {
    let num_files = parsed
        ...
        .and_then(serde_json::Value::as_u64)
        .unwrap_or(0);
    format!("{icon} Found {num_files} files")
}
...
fn format_grep_result(icon: &str, parsed: &serde_json::Value) -> String {
    ...
    let num_files = parsed
        ...
        .and_then(serde_json::Value::as_u64)
        .unwrap_or(0);
    format!("{icon} Found {num_matches} matches in {num_files} files")
}

fn format_generic_tool_result(icon: &str, name: &str, _parsed: &serde_json::Value) -> String {
    format!("{icon} Completed {name}")
}
...
impl CliToolExecutor {
    ...
    fn execute_search_tool(&self, value: serde_json::Value) -> Result<String, ToolError> {
        ...
        let output = self.tool_registry.search(
            &input.query,
            input.max_results.unwrap_or(5),
            pending_mcp_servers,
        );
        let output_value =
            serde_json::to_value(&output).map_err(|error| ToolError::new(error.to_string()))?;
        let matches = output_value["matches"]
            .as_array()
            .ok_or_else(|| ToolError::new("ToolSearch returned invalid matches"))?
            .iter()
            .filter_map(serde_json::Value::as_str);
        let mut exposed_tools = self
            .exposed_tools
            .lock()
            .unwrap_or_else(std::sync::PoisonError::into_inner);
        for name in matches {
            ...
            {
                exposed_tools.insert(name.to_string());
            }
        }
        ...
        match result {
            Ok(output) => {
                if self.emit_output {
                    ...
                    with_terminal_output(|stdout| {
                        self.renderer.stream_markdown(&markdown, stdout)?;
                        writeln!(stdout)
                    })
                    .map_err(|error| ToolError::new(error.to_string()))?;
                }
...
#[allow(clippy::too_many_lines)]
fn print_help_body(out: &mut impl Write) -> io::Result<()> {
    writeln!(out, "oog v{VERSION}")?;
    writeln!(out)?;
    writeln!(out, "Usage:")?;
    writeln!(
        out,
        "  minseo [--model MODEL] [--allowedTools TOOL[,TOOL...]]"
    )?;
    writeln!(out, "      Start the interactive REPL")?;
    writeln!(
        out,
        "  minseo [--model MODEL] [--output-format text|json] prompt TEXT"
    )?;
    writeln!(out, "      Send one prompt and exit")?;
    writeln!(
        out,
        "  minseo [--model MODEL] [--output-format text|json] TEXT"
    )?;
    writeln!(out, "      Shorthand non-interactive prompt mode")?;
    writeln!(
        out,
        "  oog --resume [SESSION.jsonl|session-id|latest] [/status] [/compact] [...]"
    )?;
    writeln!(
        out,
        "      Inspect or maintain a saved session without entering the REPL"
    )?;
    writeln!(out, "  oog help")?;
    writeln!(out, "      Alias for --help")?;
    writeln!(out, "  oog version")?;
    writeln!(out, "      Alias for --version")?;
    writeln!(out, "  oog status")?;
    writeln!(
        out,
        "      Show the current local workspace status snapshot"
    )?;
    writeln!(out, "  oog sandbox")?;
    writeln!(out, "      Show the current sandbox isolation snapshot")?;
    writeln!(out, "  minseo dump-manifests")?;
    writeln!(out, "  minseo bootstrap-plan")?;
    writeln!(out, "  oog agents")?;
    writeln!(out, "  oog mcp")?;
    writeln!(out, "  oog skills")?;
    writeln!(
        out,
        "  minseo system-prompt [--cwd PATH] [--date YYYY-MM-DD]"
    )?;
    writeln!(out, "  minseo login")?;
    writeln!(out, "  minseo logout")?;
    writeln!(out, "  oog init")?;
    writeln!(out)?;
    writeln!(out, "Flags:")?;
    writeln!(
        out,
    ...
    writeln!(out, "{}", render_live_slash_command_help())?;
    writeln!(out)?;
    ...
    writeln!(
        out,
        "  REPL turns auto-save to .minseo/sessions/<session-id>.{PRIMARY_SESSION_EXTENSION}"
    )?;
    writeln!(
        out,
    ...
    writeln!(out, "  oog --model claude-opus \"summarize this repo\"")?;
    writeln!(
        out,
        "  oog --output-format json prompt \"explain src/main.rs\""
    )?;
    writeln!(
        out,
        "  oog --allowedTools read,glob \"summarize Cargo.toml\""
    )?;
    writeln!(out, "  oog --resume {LATEST_SESSION_REFERENCE}")?;
    writeln!(
        out,
        "  oog --resume {LATEST_SESSION_REFERENCE} /status /diff /export notes.txt"
    )?;
    writeln!(out, "  oog agents")?;
    writeln!(out, "  oog mcp show my-server")?;
    writeln!(out, "  oog /skills")?;
    writeln!(out, "  minseo login")?;
    writeln!(out, "  oog init")?;
    Ok(())
...
fn print_help_to(out: &mut impl Write) -> io::Result<()> {
    ...
    print_help_body(&mut help)?;
    let help = String::from_utf8(help)
        .expect("help text is UTF-8")
        .replace("minseo", "oog");
...
mod tests {
    use super::{
        auto_session_title, build_runtime_plugin_state_with_loader,
        build_runtime_with_plugin_state, create_managed_session_handle, default_tool_profile,
        delete_managed_session, describe_tool_progress, detect_project_stack, ensure_session_title,
        ...
        format_pr_report, format_resume_report, format_small_report, format_status_report,
        format_tool_call_start, format_tool_result, format_turn_result, format_ultraplan_report,
        format_unknown_slash_command, format_unknown_slash_command_message,
        normalize_permission_mode, normalize_session_title, parse_args as parse_cli_args,
        parse_git_status_branch, parse_git_status_metadata_for, parse_git_workspace_summary,
        permission_policy, print_help_to, push_output_block, read_session_title,
        render_config_report, render_diff_report, render_diff_report_for, render_memory_report,
        render_repl_help, render_resume_usage, render_section_start, render_session_picker,
        resolve_model_alias, resolve_session_reference, response_to_events,
        resume_supported_slash_commands, run_resume_command, session_picker_window,
        session_preview, slash_command_completion_candidates_with_sessions, status_context,
        suggest_slash_commands, validate_no_args, visible_message_text, write_mcp_server_fixture,
        write_session_title, CliAction, CliOutputFormat, CliToolExecutor, GitWorkspaceSummary,
        InternalPromptProgressEvent, InternalPromptProgressState, LiveCli, ManagedSessionSummary,
        SessionHandle, SlashCommand, StatusUsage, CORE_TOOL_NAMES, DEFAULT_MODEL,
    };
    ...
    use runtime::{
        ...
        PermissionMode, Session, TokenUsage, ToolExecutor, UsageTracker,
    };
    use serde_json::json;
    use std::fs;
    use std::path::{Path, PathBuf};
    ...
    fn parse_args(args: &[String]) -> Result<CliAction, String> {
        ...
        let previous = std::env::var_os("MINSEO_CONFIG_HOME");
        std::env::set_var("MINSEO_CONFIG_HOME", &config_home);
        let result = parse_cli_args(args);
        match previous {
            Some(value) => std::env::set_var("MINSEO_CONFIG_HOME", value),
            None => std::env::remove_var("MINSEO_CONFIG_HOME"),
        }
    ...
    fn write_plugin_fixture(root: &Path, name: &str, include_hooks: bool, include_lifecycle: bool) {
        ...
        if include_lifecycle {
            ...
            let (extension, init_script, shutdown_script) = if cfg!(windows) {
                (
                    "cmd",
            ...
            fs::write(
                root.join("lifecycle").join(format!("init.{extension}")),
                init_script,
            )
            .expect("write init lifecycle");
            fs::write(
                root.join("lifecycle").join(format!("shutdown.{extension}")),
                shutdown_script,
            )
    ...
    fn default_permission_mode_uses_project_config_when_env_is_unset() {
        ...
        std::fs::create_dir_all(cwd.join(".minseo")).expect("project config dir should exist");
        std::fs::create_dir_all(&config_home).expect("config home should exist");
        std::fs::write(
            cwd.join(".minseo").join("settings.json"),
            r#"{"permissionMode":"acceptEdits"}"#,
        )
        .expect("project config should write");

        let original_config_home = std::env::var("MINSEO_CONFIG_HOME").ok();
    ...
    fn parses_direct_agents_mcp_and_skills_slash_commands() {
        ...
        assert!(error.contains("oog --resume SESSION.jsonl /status"));
    }
    ...
    #[test]
    fn suggestions_do_not_expose_hidden_commands() {
        let suggestions = suggest_slash_commands("voice");
        assert!(!suggestions.iter().any(|suggestion| suggestion == "/voice"));
    }
    ...
    fn rejects_unknown_options_with_helpful_guidance() {
        ...
        assert!(error.contains("oog --help"));
    }
    ...
    fn filtered_tool_specs_respect_allowlist() {
        let allowed = ["read_file", "grep_search"]
            ...
            .map(str::to_string)
            .collect();
        let exposed = default_tool_profile(Some(&allowed));
        let filtered = filter_tool_specs(&GlobalToolRegistry::builtin(), Some(&allowed), &exposed);
        let names = filtered
            .into_iter()
            .map(|spec| spec.name)
            .collect::<Vec<_>>();
    ...
    fn filtered_tool_specs_include_plugin_tools() {
        let exposed = default_tool_profile(None);
        let filtered = filter_tool_specs(&registry_with_plugin_tool(), None, &exposed);
        let names = filtered
            .into_iter()
    ...
    #[test]
    fn default_tool_profile_has_only_the_six_core_tools() {
        assert_eq!(
    ...
    #[test]
    fn small_report_separates_provider_tokens_from_local_estimates() {
        ...
        let exposed = default_tool_profile(None);
        let report = format_small_report(
            &["Oog stays concise.".to_string()],
            &session,
            &UsageTracker::from_session(&session),
            &GlobalToolRegistry::builtin(),
            None,
            &exposed,
        );

        assert!(report.contains("Active tools     6"));
        assert!(report.contains("Latest input     120"));
        assert!(report.contains("Latest output    8"));
        assert!(report.contains("Latest cache in  5"));
    ...
    #[test]
    fn tool_search_exposes_discovered_tools_for_the_next_request() {
        let exposed_tools = Arc::new(Mutex::new(default_tool_profile(None)));
        ...
        executor
            .execute("ToolSearch", r#"{"query":"web current"}"#)
            .expect("tool search should succeed");

        assert!(exposed_tools
            .lock()
            .expect("exposure state should not be poisoned")
            .contains("WebSearch"));
    }
    ...
    #[test]
    fn repl_help_lists_the_compact_oog_control_surface() {
        let help = render_repl_help();
        assert!(help.contains("Oog controls"));
        assert!(help.contains("/help"));
        assert!(help.contains("/see"));
        assert!(help.contains("/smash"));
        assert!(help.contains("/small"));
        ...
        assert!(!help.contains("/bughunter"));
    }
    ...
    #[test]
    fn startup_banner_keeps_only_useful_oog_context() {
        let _guard = env_lock();
        ...
        assert!(banner.contains("___    ___    ____"));
        assert!(banner.contains("carve less. ship more."));
        assert!(banner.contains("Ask Oog to build, fix, or explain something."));
        assert!(banner.contains("Saved chats: /session"));
        assert!(banner.contains("Model:"));
        assert!(banner.contains("Permissions:"));
        assert!(banner.contains("Directory:"));
        assert!(!banner.contains("Recent activity:"));
    ...
    fn init_help_mentions_direct_subcommand() {
        ...
        assert!(help.contains("oog help"));
        assert!(help.contains("oog version"));
        assert!(help.contains("oog status"));
        assert!(help.contains("oog sandbox"));
        assert!(help.contains("oog init"));
        assert!(help.contains("oog agents"));
        assert!(help.contains("oog mcp"));
        assert!(help.contains("oog skills"));
        assert!(help.contains("oog /skills"));
    }
    ...
    #[test]
    fn project_stack_detects_this_rust_workspace() {
        assert_eq!(
            detect_project_stack(Path::new(env!("CARGO_MANIFEST_DIR"))),
            "Rust"
        );
    }

    #[test]
    fn session_titles_and_previews_are_compact() {
        assert_eq!(
            normalize_session_title("  Fix login flow  ").unwrap(),
            "Fix login flow"
        );
        assert!(normalize_session_title("bad\ntitle").is_err());

        let mut session = Session::new();
        session
            .push_user_text("Investigate the login redirect loop")
            .unwrap();
        assert_eq!(
            session_preview(&session).as_deref(),
            Some("Investigate the login redirect loop")
        );
        assert_eq!(
            auto_session_title(&session).as_deref(),
            Some("Investigate the login redirect loop")
        );
    }

    #[test]
    fn automatic_title_preserves_a_manual_title() {
        let directory = temp_dir();
        fs::create_dir_all(&directory).unwrap();
        let path = directory.join("session.jsonl");
        write_session_title(&path, "Manual title").unwrap();

        let mut session = Session::new();
        session.push_user_text("Automatic title").unwrap();
        ensure_session_title(&path, &session).unwrap();

        assert_eq!(read_session_title(&path).as_deref(), Some("Manual title"));
        fs::remove_dir_all(directory).unwrap();
    }

    #[test]
    fn saved_transcript_omits_tool_payloads() {
        let message = ConversationMessage {
            ...
            blocks: vec![
                ...
                ContentBlock::ToolUse {
                    ...
                    input: "{\"path\":\"secret.txt\"}".to_string(),
                },
            ],
            usage: None,
        };

        assert_eq!(visible_message_text(&message), "Done.");
    }

    #[test]
    fn session_picker_marks_the_selected_chat() {
        let picker = render_session_picker(
            &[ManagedSessionSummary {
                id: "session-1".to_string(),
                path: PathBuf::from("session-1.jsonl"),
                modified_epoch_millis: 0,
                message_count: 3,
                title: Some("Fix login".to_string()),
                preview: Some("Investigate redirect loop".to_string()),
        ...
        assert!(picker.contains("› Fix login"));
        assert!(picker.contains("Enter to open"));
    }

    #[test]
    fn session_picker_shows_delete_confirmation() {
        let picker = render_session_picker(&[], 0, true, 5);
        assert!(picker.contains("Delete selected chat? Y to confirm"));
    }

    #[test]
    fn session_picker_window_scrolls_to_keep_selection_visible() {
        assert_eq!(session_picker_window(0, 12, 5), (0, 5));
        assert_eq!(session_picker_window(7, 12, 5), (5, 10));
        assert_eq!(session_picker_window(11, 12, 5), (7, 12));
    }

    #[test]
    fn deleting_session_removes_its_saved_tool_output() {
        let directory = temp_dir();
        fs::create_dir_all(&directory).expect("session directory should exist");
        let handle = SessionHandle {
            id: "session-1".to_string(),
            path: directory.join("session-1.jsonl"),
        };
        fs::write(&handle.path, "{}\n").expect("session should exist");
        let artifact = handle.path.with_extension("tool-output").join("tool-1.txt");
        fs::create_dir_all(artifact.parent().expect("artifact parent"))
            .expect("artifact directory should exist");
        fs::write(&artifact, "full command output").expect("artifact should exist");

        delete_managed_session(&handle).expect("session should delete");
...
UU conflicted.rs",
    ...
    fn help_mentions_jsonl_resume_examples() {
        ...
        print_help_to(&mut help).expect("help should render");
        let help = String::from_utf8(help).expect("help should be utf8");
        assert!(help.contains("oog --resume [SESSION.jsonl|session-id|latest]"));
        assert!(help.contains("Use `latest` with --resume, /resume, or /session switch"));
        assert!(help.contains("oog --resume latest"));
        assert!(help.contains("oog --resume latest /status /diff /export notes.txt"));
    }
    ...
    fn managed_sessions_default_to_jsonl_and_resolve_legacy_json() {
        ...
        let legacy_path = workspace.join(".minseo/sessions/legacy.json");
    ...
    fn resume_usage_mentions_latest_shortcut() {
        ...
        assert!(usage.contains(".minseo/sessions/<session-id>.jsonl"));
    ...
    fn temp_workspace(label: &str) -> PathBuf {
        let nanos = std::time::SystemTime::now()
            ...
            .as_nanos();
        std::env::temp_dir().join(format!("minseo-cli-{label}-{nanos}"))
    }
    ...
    fn tool_rendering_helpers_compact_output() {
        ...
        assert!(start.contains("Oog look:"));
        assert!(start.contains("look:"));
        assert!(start.contains("src/main.rs"));
        assert!(
            format_tool_call_start("PowerShell", r#"{"command":"Get-ChildItem"}"#)
                .contains("Oog bonk: Get-ChildItem")
        );
        ...
        assert!(done.contains("Read src/main.rs (1 lines)"));
        assert!(!done.contains("hello"));

        let failed = format_tool_result("read_file", "permission denied", true);
        assert!(failed.contains("permission denied"));
        assert!(!failed.contains('\n'));
    }

    #[test]
    fn tool_sections_keep_one_blank_line_between_output_and_rule() {
        let mut trailing_newlines = 0;
        let first = render_section_start("Oog look: src/main.rs", &mut trailing_newlines);
        assert!(first.starts_with("\n\n"));
        assert!(first.contains("Oog look: src/main.rs"));

        let second = render_section_start("Oog bonk: cargo test", &mut trailing_newlines);
        assert!(second.starts_with('\n'));
        assert!(!second.starts_with("\n\n"));
    }

    #[test]
    fn turn_result_reports_files_tests_time_and_usage() {
        ...
        let result = format_turn_result(&summary, Duration::from_secs(2), "claude-sonnet")
            .expect("tool turn should have a result");
        assert!(result.contains("Oog done · 2 tools"));
        assert!(result.contains("carved src/lib.rs"));
        assert!(result.contains("tests passed"));
        assert!(result.contains("2s"));
        assert!(result.contains("12 in / 4 out"));

        let failed_summary = runtime::TurnSummary {
            tool_results: vec![ConversationMessage::tool_result(
                "test",
                "bash",
                r#"{"exitCode":1}"#,
        ...
        assert!(
            format_turn_result(&failed_summary, Duration::from_secs(2), "claude-sonnet")
                .expect("tool turn should have a result")
    ...
    #[test]
    fn tool_rendering_hides_large_success_output() {
        let content = (0..200)
        ...
        assert!(rendered.contains("Read src/main.rs (200 lines)"));
    ...
    fn build_runtime_runs_plugin_lifecycle_init_and_shutdown() {
        ...
        assert!(
            runtime_plugin_state
                ...
                .expect("installed plugin should be discovered")
                .is_enabled(),
            "installed plugin should retain its enabled state"
        );
