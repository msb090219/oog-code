...
const GITIGNORE_COMMENT: &str = "# Oog Code local artifacts";
const GITIGNORE_ENTRIES: [&str; 2] = [".minseo/settings.local.json", ".minseo/sessions/"];
...
pub(crate) fn initialize_repo(cwd: &Path) -> Result<InitReport, Box<dyn std::error::Error>> {
    ...
    let claude_dir = cwd.join(".minseo");
...
pub(crate) fn render_init_claude_md(cwd: &Path) -> String {
    ...
    let mut lines = vec![
        "# CLAUDE.md".to_string(),
        String::new(),
        "This file provides guidance to Oog Code by Troglodytic when working with code in this repository.".to_string(),
        String::new(),
    ];
...
mod tests {
    ...
    fn initialize_repo_creates_expected_files_and_gitignore_entries() {
        ...
        assert!(gitignore.contains(".minseo/settings.local.json"));
        assert!(gitignore.contains(".minseo/sessions/"));
    ...
    fn initialize_repo_is_idempotent_and_preserves_existing_files() {
        ...
        fs::write(root.join(".gitignore"), ".minseo/settings.local.json\n")
        ...
        assert!(second_rendered.contains(".minseo/         skipped (already exists)"));
        ...
        let gitignore = fs::read_to_string(root.join(".gitignore")).expect("read gitignore");
        assert_eq!(gitignore.matches(".minseo/settings.local.json").count(), 1);
        assert_eq!(gitignore.matches(".minseo/sessions/").count(), 1);
