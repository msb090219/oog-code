...
cargo build -p rusty-Codex-cli --release

# Run the CLI (after building)
./target/release/minseo
...
cargo test -p rusty-Codex-cli --test mock_parity_harness -- --nocapture
...
cargo run -p rusty-Codex-cli

# One-shot prompt
cargo run -p rusty-Codex-cli -- prompt "explain this codebase"

# With specific model
cargo run -p rusty-Codex-cli -- --model sonnet prompt "fix the bug"
```

## Architecture Overview

This is a Rust workspace implementing a high-performance CLI AI coding assistant called **Minseo Code**.

### Workspace Structure

```
crates/
├── rusty-Codex-cli/     # Main CLI binary (REPL, one-shot prompts, args)
...
```
rusty-Codex-cli (main entry point)
...
**Monolithic Main Structure**: The `rusty-Codex-cli/src/main.rs` is large (~3,000 lines) and orchestrates:
...
**Entry Point**: Binary name is `minseo`, defined in `crates/rusty-Codex-cli/Cargo.toml`.

**Authentication**: Supports both API key (`MINSROPIC_API_KEY` env var) and OAuth PKCE flow (`minseo login`).

**Model Aliases**: Short names resolve to latest versions:
- `opus` → `Codex-opus-4-6`
- `sonnet` → `Codex-sonnet-4-6`
- `haiku` → `Codex-haiku-4-5-20251213`
...
- `MINSROPIC_API_KEY` - API authentication
- `MINSROPIC_BASE_URL` - Optional proxy/base URL (e.g., for Zhipu AI GLM)

Config file hierarchy: `.Codex.json` (not present by default but supported).
